...
# THƯ VIỆN CHO TRÌNH DUYỆT VÀ CAPTCHA
...
class CodeStorageManager:
    ...
    def load_data(self):
        ...
        if not os.path.exists(self.filename):
            try: open(self.filename, 'w').close()
            except: pass
            return data
        try:
            ...
            for line in lines:
                ...
                if current_section: data[current_section].append(line)
        except: pass
        return data
        ...
        try:
            with open(self.filename, 'w', encoding='utf-8') as f:
                ...
                for vip_type in sorted_keys:
                    ...
                    f.write(f"{vip_type}\n")
                    for code in codes: f.write(f"{code}\n")
                    f.write("\n")
            return True
        except: return False
    ...
    def add_codes(self, new_codes_dict):
        ...
        for vip, codes in new_codes_dict.items():
            if not codes: continue
            vip_upper = vip.upper()
            if vip_upper not in data: data[vip_upper] = []
    ...
    def pop_codes(self, vip_type, amount):
        ...
        if vip_upper not in data: return None, 0
        current_list = data[vip_upper]
        if len(current_list) < amount: return None, len(current_list)
...
class ProxyManager:
    def __init__(self, proxy_list=None):
        ...
        if proxy_list: self.load_proxies_from_list(proxy_list)
    ...
    def mark_bad(self, proxy):
        if proxy and proxy not in self.bad_proxies: self.bad_proxies.add(proxy)
...
def download_image(url, save_path):
    ...
    try:
        if url.startswith("data:image"):
            ...
            with open(save_path, "wb") as f: f.write(base64.b64decode(encoded))
            return True
        else:
...
async def feedback_loop_drag_async(page, start_x, start_y, target_piece_x, initial_green_x, thread_id="1"):
    ...
    for _ in range(200): 
        ...
        if distance_left > 0:
            if distance_left > 40: step = random.uniform(8, 15)
            elif distance_left > 10: step = random.uniform(3, 6)
            else: step = random.uniform(0.5, 2)
...
async def auto_drag_slider_async(page, thread_id="1"):
    ...
    while True:
        if attempt >= MAX_ATTEMPTS:
            print(f"[Luồng {thread_id}] ❌ Hủy luồng Playwright: Đã thất bại {MAX_ATTEMPTS} lần liên tiếp.")
            return False
        ...
        try:
            ...
            if not await bg_locator.is_visible() or not await piece_locator.is_visible(): continue

            bg_url = await bg_locator.get_attribute("src")
            piece_url = await piece_locator.get_attribute("src")
            if not bg_url or not piece_url: continue
            ...
            if not success_bg or not success_piece: continue
...
# ========================================================
# ==>> MỞ BROWSER GỬI CODE (KHÔNG PROXY, NATIVE STEALTH) <<==
# ========================================================
async def async_send_with_browser(email, worker_id="1"):
    print(f"[Luồng {worker_id}] 🌐 [NO-PROXY] Mở Playwright yêu cầu gửi SMS cho: {email}")
    ...
    try:
        ...
        # SỬ DỤNG MÃ STEALTH THUẦN (KHÔNG DÙNG THƯ VIỆN PIP NỮA ĐỂ TRÁNH LỖI)
...
# ==============================================================
# ==>> BROWSER HOST (TREO ACC CHỦ NATIVE STEALTH) <<==
# ==============================================================
...
def safe_request(method, url, proxy, **kwargs):
    ...
    try:
        if method.lower() == 'get': resp = requests.get(url, proxies=proxies_dict, **kwargs)
        else: resp = requests.post(url, proxies=proxies_dict, **kwargs)
...
def get_temp_email(proxy):
    ...
    try:
        ...
        domains_data = r.json()
        if "hydra:member" not in domains_data or not domains_data["hydra:member"]: return None, None
        ...
        if r_acc.status_code not in [200, 201]: return None, None
...
def login_vmos(email, code, proxy, invite_code=None, user_agent=None):
    ...
    headers = {
        ...
        "referer": "https://cloud.vmoscloud.com/", "User-Agent": ua, 
...
def trigger_agent_activation(token, userid, proxy, headers):
    ...
    try: safe_request("POST", save_agent_url, proxy, headers=headers, json=payload)
...
def exchange_target_gem(token, userid, proxy, target_id):
    ...
    for attempt in range(5): 
        try:
            ...
            if resp and resp.status_code == 200:
                data = resp.json()
                if data.get("code") == 200: return True
        except: pass
        time.sleep(2)
    return False
...
async def task_worker(invite_code, update_callback=None):
    ...
    while True:
        ...
        try:
            ...
            if update_callback: await update_callback(f"📩 Đang khởi chạy Playwright gửi OTP tới {email}...")
            ...
            if update_callback: await update_callback(f"⏳ Đang chờ mã OTP từ Inbox...")
...
async def use_code(ctx):
    ...
    total_codes = 0
    if not data: embed.description = "Kho đang trống trơn! Hãy chạy `!genbuff1` để kiếm thêm."
    else:
    ...
    try:
        ...
        if match:
            ...
            if codes is None: await ctx.send(f"❌ Không tìm thấy `{vip_req.upper()}`")
            elif len(codes) < count_req: await ctx.send(f"⚠️ Không đủ hàng!")
            else:
                codes_str = "\n".join(codes)
                await ctx.send(f"✅ **Đã lấy {count_req} {vip_req.upper()}** (Còn: {remaining}):\n```\n{codes_str}\n```")
...
async def genbuff(ctx, arg1: str = None, arg2: str = None):
    ...
    try:
        ...
        if vip_type == 'all':
            ...
            target_vip_types = [k for k in VIP_MAP.keys() if k not in parts[1:]]
            display_type = f"ALL (Trừ: {', '.join([e.upper() for e in parts[1:]])})"
        ...
        host_idx = 1
        if is_inf_mode: is_inf_running = True
        
        while True:
            if is_inf_mode and not is_inf_running: break
            if not is_inf_mode and host_idx > num_hosts: break 
            ...
            if not host_acc: continue
            
            host_token, host_userid, host_proxy = host_acc['token'], host_acc['userId'], host_acc['proxy_used']
            ...
            embed_run.add_field(name="🎟️ Link Mời", value=f"Mã: `{invite_code}`", inline=False)
            embed_run.add_field(name="📊 Tiến độ Buff", value=f"Đang chạy {CONFIG['NUM_THREADS']} luồng...", inline=False)
            ...
            current_assets_num, total_success_local, total_fail = 0, 0, 0
            
            async def protected_worker():
                async with semaphore: return await task_worker(invite_code=invite_code)

            active_tasks = set()
            for _ in range(concurrency): active_tasks.add(asyncio.create_task(protected_worker()))
                
            last_update_time, is_host_failed = 0, False
            ...
            while active_tasks:
                ...
                for task in done:
                    ...
                    if proxy_manager.get_live_count() > 0:
                        while len(active_tasks) < concurrency: active_tasks.add(asyncio.create_task(protected_worker()))
                    else:
                        if not active_tasks:
                            if is_inf_mode:
                                await asyncio.sleep(60) 
            ...
            if current_assets_num >= num_buffs:
                ...
                await msg.edit(embed=embed_run)
                for ex_id in exchange_list: await asyncio.to_thread(exchange_target_gem, host_token, host_userid, host_proxy, ex_id)
                ...
                if codes_dict:
                    ...
                    lines = [f"{k.upper():<5}: {len(all_data[k])}" for k in sorted(all_data.keys(), key=lambda x: (len(x), x))]
                    final_code_str = "\n".join(lines) if lines else "Kho trống."
...
if __name__ == "__main__":
    ...
    if token_local:
        ...
        print(f"🚀 Bot Pro Ultimate + Playwright (Native Stealth) đang chạy...")
        while True:
            try: bot.run(token_local)
